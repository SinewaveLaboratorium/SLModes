    ...
    def compare(self, scale02):
        ...
        for i in range(0, 7):  # for each of the note of the user scale
            for j in range(0, 7):  # compare it with every note of current scale of the scales_list
                if ((self[i].midi_note() == scale02[j].midi_note()) or (self[i].midi_note() == scale02[j].midi_note() - 12) or (self[i].midi_note() == scale02[j].midi_note() + 12)):
                    count = count + 1
                    break
        ...
        if (self.notes[1] in scale02.notes) and (self.notes[1] in scale02.notes) and (self.notes[2] in scale02.notes) and (self.notes[3] in scale02.notes) and (self.notes[4] in scale02.notes) and (self.notes[5] in scale02.notes) and (self.notes[6] in scale02.notes):
            return True
        else:
            return False
    ...
    def in_scale(self, sc):
        ...
        for i in self.notes: #for every note of the scale
            for j in range(0, 7):  # check every note of the scale
                if i.midi_note() == sc[j].midi_note() or i.midi_note() == sc[j].midi_note() + 12 or i.midi_note() == sc[j].midi_note() - 12:
