...
from midiutil.MidiFile import MIDIFile ## midiutil by Mark Conway Wirt uses a Open Source License that can be found at the folder Third-Party Library Licenses
...
def load_temp_chord_progressions(Chord_Progression_Listbox, Chord_Progression_Modes_Listbox, path):

    try:
        temp_chord_progression = open(path, 'r', encoding='utf-8')
...
def save_temp_chord_progressions(Chord_Progression_Modes_Listbox, Chord_Progression_Listbox, path):
    save_text = ""
    ...
    temp_chord_progression = open(path, 'w', encoding='utf-8')
...
def play_chord_progression(composer_mode_window, PLAY_STYLE_SETTING, Chord_Progression_Listbox, Chord_Progression_Modes_Listbox):
    ...
    if settings.is_playing == False and settings.is_play_stopped == True:
        MN_AUDIO.playComposerChords(Chord_Progression_Listbox, Chord_Progression_Modes_Listbox, style, composer_mode_window)
