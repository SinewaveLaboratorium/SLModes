...
SUSTAIN_SETTING = int(parser.get('user-settings', 'sustain'))
...
window.geometry("980x540+300+300")
...
freboardfile = PhotoImage(file=freboarddpath)
pianofile = PhotoImage(file=pianopath)
notefile = PhotoImage(file=fretboardnotepath)
largenotefile = PhotoImage(file=freboarddpathlarge)
largerootnotefile = PhotoImage(file=freboarddpathrootnote)
largechordnotefile = PhotoImage(file=freboarddchordnote)
extendedfreboardfile = PhotoImage(file=extendedfreboardpath)
...
fretboard01_canvas.create_image(97, 57, image=freboardfile)
...
fretboard02_canvas.create_image(97, 57, image=freboardfile)
...
def show_midi_error():
    ...
    error_window.geometry("338x150+550+450")
...
sustaincheckbox = SUSTAIN_SETTING
cb = IntVar()
cb.set(FLAT_SETTING)
sus = IntVar()
sus.set(SUSTAIN_SETTING)
maj_scale = IntVar()
...
def Open_Settings():
    ...
    if settings.midi_port != -1:
        midi_out_box = ttk.Combobox(top, textvariable=midivar, values=settings.midi_ports_list, state='readonly',
                                    width=25)
        ...
        midi_out_box = ttk.Combobox(top, values=midi_no_ports, width=25)
        midi_out_box.config(font=font_01)
        midi_out_box.current(0)

    midi_out_box.place(x=85, y=65)

    checkbox = Checkbutton(top, variable=cb, command=update_checkbox, bg="white")  # Show flats
    checkbox.bind()
    checkbox.place(x=205, y=95)
    checkbox.config(font=font_01)

    sustaincheckbox = Checkbutton(top, variable=sus, command=update_sus, bg="white")
    sustaincheckbox.bind()
    sustaincheckbox.place(x=205, y=125)
    sustaincheckbox.config(font=font_01)
...
# Chords Checkboxes
    ...
    basic_scales_button = Button(top, text="-", width=7, highlightbackground="black", command=lambda: process_scales_button("-"))
    basic_scales_button.place(x=80, y=553)

    more_scales_button= Button(top, text="+", width=7, highlightbackground="black", command=lambda: process_scales_button("+"))
    more_scales_button.place(x=153, y=553)

    all_scales_button= Button(top, text="++", width=7, highlightbackground="black", command=lambda: process_scales_button("++"))
    ...
    basic_chords_button= Button(top, text="-", width=7, highlightbackground="black", command=lambda: process_chords_button("-"))
    basic_chords_button.place(x=487, y=553)

    more_chords_button= Button(top, text="+", width=7, highlightbackground="black", command=lambda: process_chords_button("+"))
    more_chords_button.place(x=560, y=553)

    all_chords_button= Button(top, text="++", width=7, highlightbackground="black", command=lambda: process_chords_button("++"))
...
def read_root(event):
    ...
    if is_extended_fretboard_open == True:
        ...
        extended_fretboard_canvas.create_image(490,215,image=extendedfreboardfile)
...
def read_scale(event):
    ...
    UserScale = Scale(Note(Root_in), Scale_in)
    drawModeShape(Scale_in, fretboard01_canvas, notefile, freboardfile)
    drawPianoMode(UserScale, piano01_canvas, notefile, pianofile)
    show_scale_notes(True, scale01_notes_label)
...
#### UPDATE Functions #####
def update_sus():
    global SUSTAIN_SETTING
    parser.set('user-settings', 'sustain', str(sus.get()))
    with open('user-settings.txt', 'w') as configfile:
        parser.write(configfile)

    if SUSTAIN_SETTING == 0:
        SUSTAIN_SETTING = 1
    else:
        SUSTAIN_SETTING = 0
...
def create_mode_chords_right_up():
    ...
    if a != (""):
        ...
        drawModeShape(scale_name, fretboard02_canvas, notefile, freboardfile)
...
def play_and_show_mode_chords_left_up_right_up(event, scale_list, chord_list, text, frame, md, r, s):
    ...
    if text == "Matched_Chords2":
        enable_second_fretboard_drawing = True
    MN_MIDI.playMatchedChord(event, scale_list, chord_list, text, SUSTAIN_SETTING, md, r, s)
...
def play_and_show_scale_chords_left_down_right_down(event, chord_list, seven_list, frame, md): # This plays the SCALE CHORDS both on the LEFT and RIGHT
    ...
    global enable_second_fretboard_drawing
    # enable_second_fretboard_drawing = True
    MN_MIDI.playMoreChord(event, chord_list, seven_list, SUSTAIN_SETTING, md)
    show_Chord_Notes(event, frame, chord_list)
    if show_chords == True and is_extended_fretboard_open == True:
        extended_fretboard_canvas.delete("all")
        extended_fretboard_canvas.create_image(490, 215, image=extendedfreboardfile)
...
def Expand_Fretboard():
    ...
    if is_extended_fretboard_open == False:
        ...
        extended_fretboard_window.geometry("980x450+550+400")
...
def Open_Composer_Mode():
    ...
    if is_composer_mode_open == False:
        ...
        composer_mode_window.geometry("660x370+550+400")
        ...
        Composer_Mode_utils.Play = Button(composer_mode_window, text="Play", width=7, bg='#54FA9B',  highlightbackground="black", command=lambda: Composer_Mode_utils.play_chord_progression(composer_mode_window, PLAY_STYLE_SETTING, Chord_Progression_Listbox, Chord_Progression_Modes_Listbox, SUSTAIN_SETTING, midiout))
