...
def playscale(scale, sustain, midiout):
...
def playMatchedChord(event, myListbox_Scale, myListbox_Chords, myListBoxName, sustain, midiout, Root_in, Scale_in):
    ...
    if settings.midi_port != -1:
        ...
        if myListBoxName == "Matched_Chords2":
            ...
            time.sleep(0.5)
            threading.Thread(target=playscale, args=(MN_Scales.Scale(root, scale), sustain, midiout)).start()
        else:
            a = MN_Scales.Chord(myListbox_Chords.get(ANCHOR))
            threading.Thread(target=playchord, args=(str(a), midiout)).start()
            time.sleep(0.5)
            threading.Thread(target=playscale, args=(MN_Scales.Scale(Root_in, Scale_in), sustain, midiout)).start()

def playMoreChord(event, myListbox_Chords, seven_scale_list, sustain, midiout):
    ...
    if settings.midi_port != -1:
        ...
        for i in range(0, len(seven_scale_list)):
            if a.notes[0].midi_note() == seven_scale_list[i].notes[0].midi_note(): # CAN THIS BE IMPROVED?
                threading.Thread(target=playscale, args=(seven_scale_list[i], sustain, midiout)).start()

def playComposerChords(listbox1, listbox2, sustain, style, window, midiout):
